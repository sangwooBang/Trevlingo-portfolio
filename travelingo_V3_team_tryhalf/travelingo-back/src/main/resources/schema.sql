-- ============================================
-- TravelLingo Database Schema v1.1
-- 실행방법: mysql -u root -p travellingo < schema.sql
-- 또는 MySQL Workbench에서 File→Open→이 파일→번개버튼
-- ============================================

CREATE DATABASE IF NOT EXISTS travellingo DEFAULT CHARACTER SET utf8mb4;
USE travellingo;
...
CREATE TABLE IF NOT EXISTS users (
    ...
    user_level    TINYINT      NOT NULL DEFAULT 1 COMMENT '1~3단계',
    weekly_goal   TINYINT      NOT NULL DEFAULT 5 COMMENT '주간목표시간(3/5/7)',
...
CREATE TABLE IF NOT EXISTS chapter (
    ...
    chapter_no       TINYINT      NOT NULL COMMENT 'Ch1~Ch10',
    title            VARCHAR(100) NOT NULL,
    persona_setting  TEXT         COMMENT 'AI 페르소나 프롬프트',
    total_sessions   TINYINT      NOT NULL DEFAULT 10,
    created_at       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id)
...
CREATE TABLE IF NOT EXISTS learning_content (
    ...
    session_no   TINYINT      NOT NULL COMMENT '1~10',
    session_name VARCHAR(50)  NOT NULL COMMENT '예약확인/체크인 등',
    type         ENUM('WORD','EXPRESSION','QNA') NOT NULL,
    priority     TINYINT      NOT NULL DEFAULT 1 COMMENT '1=높음 3=낮음',
...
CREATE TABLE IF NOT EXISTS conv_session (
    ...
    chapter_id    INT          NOT NULL,
    session_no    TINYINT      NOT NULL,
    persona       VARCHAR(100) NOT NULL COMMENT '적용 페르소나',
    message_count TINYINT      NOT NULL DEFAULT 0 COMMENT '최대10',
...
-- ── 샘플 데이터: 챕터 1 ─────────────────────
INSERT INTO chapter (language, category, chapter_no, title, persona_setting) VALUES
('english', '공항', 1, '출국 및 기내',
 '당신은 인천국제공항 대한항공 체크인 카운터 직원입니다. 영어로 친절하게 응대하고, 사용자가 틀린 표현을 쓰면 자연스럽게 올바른 표현으로 교정해주세요.');
