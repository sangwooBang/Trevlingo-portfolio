// ============================================================
// TravelLingo - 학습 페이지 메인 JavaScript
// ============================================================
...
// 각 챕터의 제목, 아이콘, 세션 수 정의
const allChapters = [
    { number: 1, title: '인사 & 기본',      icon: '👋', sessions: 4 },
    { number: 2, title: '공항에서',         icon: '✈️', sessions: 4 },
    { number: 3, title: '음식 & 식사',      icon: '🍽️', sessions: 5 },
    { number: 4, title: '숙박시설',         icon: '🏨', sessions: 4 },
    { number: 5, title: '쇼핑하기',         icon: '🛍️', sessions: 4 },
    { number: 6, title: '교통 & 길찾기',    icon: '🚕', sessions: 4 },
    { number: 7, title: '긴급 상황',        icon: '🆘', sessions: 3 },
    { number: 8, title: '문화 & 여가',      icon: '🎭', sessions: 4 },
    { number: 9, title: '일상 대화',        icon: '💬', sessions: 5 },
    { number: 10, title: '고급 표현',       icon: '⭐', sessions: 4 }
];
 ...
 * 1. localStorage에서 여행 정보 불러오기
 * 2. 헤더 정보 업데이트
 * 3. 사이드바 챕터 목록 렌더링
 * 4. API에서 현재 세션의 학습 콘텐츠 로드
 */
...
function updateHeaderInfo() {
    ...
    if (courseTitle && chapter) {
        courseTitle.textContent = `${chapter.icon} 영어 - Ch ${chapter.number}: ${chapter.title}`;
    }
    if (sessionTitle) {
...
function showVocabularyMode() {
    hideAllModes();
    document.getElementById('vocabulary-mode').style.display = '';
    vocabIndex = 0;
    ...
    try {
        await fetch(`${API_BASE}/progress`, {
            ...
            body: JSON.stringify({
                ...
                stepExpr: false,
                stepAi: false,
                studyDuration: Math.round((Date.now() - sessionStartTime) / 60000)
            })
        });
    } catch (e) {
...
function showExpressionsMode() {
    hideAllModes();
    document.getElementById('expressions-mode').style.display = '';
    exprIndex = 0;
...
function showAiConversationMode() {
    hideAllModes();
    document.getElementById('ai-conversation-mode').style.display = '';
    ...
    try {
        ...
        if (chatMessages) {
            ...
            aiMsgDiv.innerHTML = `
                ...
                <div class="message-content">
                    <p>
                        ${data.aiMessage || 'Sorry, I could not generate a response.'}
                        <button class="tts-inline" onclick="speakSentence('${(data.aiMessage || '').replace(/'/g, "\\'")}')">🔊</button>
                    </p>
                </div>
            `;
...
function showCompletionScreen() {
    hideAllModes();
    document.getElementById('sessionCompletion').style.display = '';
